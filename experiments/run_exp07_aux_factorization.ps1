# GENERATED FROM experiments/configs/exp07_aux_factorization.yaml - DO NOT EDIT (edit the manifest and re-run swm.exp.gen_sweep)
# exp07_aux_factorization - TRAINING ONLY, run in your own terminal (GPU + W&B online). Plan: docs/plans/2026-08-01-exp07-aux-factorization.md
# 10 cells, seed-major order so a -MaxHours cutoff leaves every cell with even seed coverage.
# Estimated per-run minutes: {'ep100': 25, 'ep40': 10} (VERIFY against the first runs; exp05-derived).
# INTERRUPT/RESUME: Ctrl-C anytime; train.resume=true + last.pt resume mid-run; DONE.txt skips finished runs.
# Usage:  cd C:\git_repo\Stellar-World-Model ; .\experiments\run_exp07_aux_factorization.ps1
#         .\experiments\run_exp07_aux_factorization.ps1 -DryRun
#         .\experiments\run_exp07_aux_factorization.ps1 -MaxHours 20.0
...
# cells: (exp_name, geometry, seed list). ALL config lives in src/swm/configs/experiment/exp07/<cell>.yaml.
$cells = @(
  @('exp07_comb0p3_off', 'w256', @(0, 1, 2, 3, 4, 5)),
  @('exp07_comb0p3_fbwd', 'w256', @(0, 1, 2, 3, 4, 5)),
  @('exp07_hann0p3_off', 'w256', @(0, 1, 2, 3, 4, 5)),
  @('exp07_hann0p3_fbwd', 'w256', @(0, 1, 2, 3, 4, 5)),
  @('exp07_lpsd0p3_off', 'w256', @(0, 1, 2, 3)),
  @('exp07_lpsd0p3_fbwd', 'w256', @(0, 1, 2, 3)),
  @('exp07_comb0p1_off', 'w256', @(0, 1, 2, 3)),
  @('exp07_comb0p1_fbwd', 'w256', @(0, 1, 2, 3)),
  @('exp07_lpsd0p1_off', 'w256', @(0, 1, 2, 3)),
  @('exp07_lpsd0p1_fbwd', 'w256', @(0, 1, 2, 3))
)
...
foreach ($r in $runs) {
  ...
  $cmd = @("+experiment=exp07/$exp", "variant=B", "seed=$seed")
  if ($DryRun) {
...
if ($failed.Count -gt 0) {
  ...
  Write-Host "SWEEP DONE (${total} h) - tell Claude Code 'exp07_aux_factorization done' to run the eval fan." -ForegroundColor Green
}
