# GENERATED FROM experiments/configs/exp08_dynamics_ladder.yaml - DO NOT EDIT (edit the manifest and re-run swm.exp.gen_sweep)
# exp08_dynamics_ladder - TRAINING ONLY, run in your own terminal (GPU + W&B online). Plan: docs/plans/2026-08-07-exp08-dynamics-ladder.md
# 6 cells, seed-major order so a -MaxHours cutoff leaves every cell with even seed coverage.
# Estimated per-run minutes: {'ep100': 25} (VERIFY against the first runs; exp05-derived).
# INTERRUPT/RESUME: Ctrl-C anytime; train.resume=true + last.pt resume mid-run; DONE.txt skips finished runs.
# Usage:  cd C:\git_repo\Stellar-World-Model ; .\experiments\run_exp08_dynamics_ladder.ps1
#         .\experiments\run_exp08_dynamics_ladder.ps1 -DryRun
#         .\experiments\run_exp08_dynamics_ladder.ps1 -MaxHours 12.0
...
# cells: (exp_name, geometry, seed list). ALL config lives in src/swm/configs/experiment/exp08/<cell>.yaml.
$cells = @(
  @('exp08_smooth', 'w256', @(0, 1, 2, 3, 4, 5)),
  @('exp08_linear', 'w256', @(0, 1, 2, 3, 4, 5)),
  @('exp08_frozen', 'w256', @(0, 1, 2, 3, 4, 5)),
  @('exp08_smooth_half', 'w256', @(0, 1, 2, 3, 4, 5)),
  @('exp08_frozen_lo', 'w256', @(0, 1, 2, 3, 4, 5)),
  @('exp08_smooth_lo', 'w256', @(0, 1, 2, 3, 4, 5))
)
...
foreach ($r in $runs) {
  ...
  $cmd = @("+experiment=exp08/$exp", "variant=B", "seed=$seed")
  if ($DryRun) {
...
if ($failed.Count -gt 0) {
  ...
  Write-Host "SWEEP DONE (${total} h) - tell Claude Code 'exp08_dynamics_ladder done' to run the eval fan." -ForegroundColor Green
}
