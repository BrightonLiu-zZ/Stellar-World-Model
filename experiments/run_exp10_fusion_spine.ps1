# GENERATED FROM experiments/configs/exp10_fusion_spine.yaml - DO NOT EDIT (edit the manifest and re-run swm.exp.gen_sweep)
# exp10_fusion_spine - TRAINING ONLY, run in your own terminal (GPU + W&B online). Plan: docs/plans/2026-08-30-exp10-fusion-spine.md
# 3 cells, seed-major order so a -MaxHours cutoff leaves every cell with even seed coverage.
# Estimated per-run minutes: {'cond_dec': 29, 'decorr': 29, 'multistep': 32} (VERIFY against the first runs; exp05-derived).
# INTERRUPT/RESUME: Ctrl-C anytime; train.resume=true + last.pt resume mid-run; DONE.txt skips finished runs.
# Usage:  cd C:\git_repo\Stellar-World-Model ; .\experiments\run_exp10_fusion_spine.ps1
#         .\experiments\run_exp10_fusion_spine.ps1 -DryRun
#         .\experiments\run_exp10_fusion_spine.ps1 -MaxHours 12.0
...
# cells: (exp_name, geometry, seed list). ALL config lives in src/swm/configs/experiment/exp10/<cell>.yaml.
$cells = @(
  @('exp10_cond_dec', 'w256', @(0, 1, 2, 3, 4, 5)),
  @('exp10_decorr', 'w256', @(0, 1, 2, 3, 4, 5)),
  @('exp10_multistep', 'w256', @(0, 1, 2, 3, 4, 5))
)
...
foreach ($r in $runs) {
  ...
  $cmd = @("+experiment=exp10/$exp", "variant=B", "seed=$seed")
  if ($DryRun) {
...
if ($failed.Count -gt 0) {
  ...
  Write-Host "SWEEP DONE (${total} h) - tell Claude Code 'exp10_fusion_spine done' to run the eval fan." -ForegroundColor Green
}
