...
| `paper/figures/figB_valloss.pdf` | generated by `experiments/plot_ml4ps_appendix_b.py` (Appendix B: validation reconstruction vs probe score, the ten exp05 recipes × four tasks). Screen copy `build/figB_valloss.png`, every plotted number `build/figB_data.csv`; row (b) is the same estimator on the shipped hann0p3 family (exp07 incumbent + 20 exp09 cells, two VOID excluded), its numbers in `build/figB_exp09_data.csv`. **Since 2026-09-20 the same script also writes `figB_valloss_a.pdf` and `figB_valloss_b.pdf`, the two rows separately** — the arXiv layout puts row (a) in the body beside the model-selection paragraph (roadmap 3.3) and keeps row (b) in Appendix E. `figB_valloss.pdf` (both rows) is still written and is no longer included by `main.tex`; it is kept so the pre-split version stays reproducible |
| `paper/figures/figD_recon.pdf` | generated by `experiments/plot_ml4ps_recon.py` (Appendix F: the within-window artifact — two stars × two recipes on a shared y-scale, plus the position-resolved impulse ratio over six seeds). Screen copy and provenance: `build/figD_recon.png`, `build/figD_recon_data.csv`. The two display stars are a random draw fixed by `--seed` (default 0 → TIC 386316688 pulsating, TIC 289475675 quiet); the lower panel reads `experiments/exp07_edge_profiles_6seed.parquet` and reproduces the published ratios (rectangular 31.7× at the edge, Hann 11.8× at the centre, per-seed maximum then seed mean). Do not hand-edit |
