...
dozen physically motivated features from these light curves, which together remain a strong baseline. Previous work has trained
...
cadences ($256 \times 120$\,s $\approx 8.5$\,h). Windows containing a missing value are discarded, never interpolated or padded,
...
with a usable window plus 10{,}000 quiet stars, split 70/15/15). No test star of the first five tasks
enters pre-training. Of the second pool's test stars about 8\% did (265 of 3{,}429), and restricting
evaluation to stars never seen in pre-training leaves all seven gains in place
(Appendix~\ref{app:unseen}).
...
readout, no significance test. $n$: test stars; prev.: positive-class prevalence.}
\label{tab:scorecard}
\centering
\scriptsize
...
of $0.920$) or the training set is too small to pay for them (RGB versus helium-burning, $-0.050$ on
...
\paragraph{The GRU prediction term accounts for most of the gain.} Removing the
...
arms are differenced seed by seed (Table~\ref{tab:readout}; largest losses solar-like
oscillators $-0.047$, rotation $-0.026$, $\nu_{\max}$ $-0.025$). The prediction term, not the
autoencoder alone, accounts for most of the summed gain over the engineered features.
...
XGBoost on the same inputs the gain exceeds $2\,\mathrm{SE}$ on 4 of 10 tasks and is negative on one
(Appendix~\ref{app:readout}). Every arm,
...
No prediction term: the same recipe trained without the GRU prediction term. Untrained: a randomly
initialised encoder, which is a single encoder and carries no band. Bold: beyond $2\,\mathrm{SE}$. The
