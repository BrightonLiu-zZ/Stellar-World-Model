...
(Figure~\ref{fig:valloss_a}). The recipe with the
lowest loss is the one without the prediction term on all four, and choosing it would cost
$0.021$--$0.049$ PR-AUC. On the family our encoder belongs to the anti-correlation does not survive as
such --- it holds on two of the four tasks and reverses on two --- but the loss optimum is still never
the probe optimum (Appendix~\ref{app:valloss}). The same objective carries a second cost: it develops
...
rather than by winning (Appendix~\ref{app:artifact}). An encoder here therefore has to be
chosen by a held-out probe, not by its own loss.

\begin{figure}[t]
\centering
\includegraphics[width=0.85\linewidth]{figures/figB_valloss_a.pdf}
\caption{Validation reconstruction loss against downstream probe score, one point per pre-training
...
correlation of $0.92$ across the ten tasks (Appendix~\ref{app:readout}). \emph{Data.} Every arm,
...
as clusters in the latent space, is untested. We leave it as an exploratory direction rather than a
...
Figure~\ref{fig:valloss_a} in \S4 is half of this measurement; Figure~\ref{fig:valloss_b} is the
other half, and the two differ in which family of recipes is varied.

In Figure~\ref{fig:valloss_a} the loss-optimal recipe is the dynamics-free one on all four tasks, and
...
$\rho$ still runs from $-0.43$ to $-0.87$. Those recipes are an earlier sweep than our encoder --
they carry an earlier spectral auxiliary term -- so Figure~\ref{fig:valloss_b} repeats the estimator
...
\caption{The same estimator as Figure~\ref{fig:valloss_a}, on the family our encoder belongs to: 19
...
PR-AUC. So the body's figure shows an anti-correlation on every task; this one shows it surviving on
two tasks and reversing on two; in neither does the loss choose. Both are statements across recipes
