...
end-to-end on the same labelled stars, splits and inputs, the combined readout is ahead of a supervised
...
than replacing them. Under a fixed linear readout, the combination matches or exceeds the two
...
Concatenating the two improves a fixed linear readout beyond $2\,\mathrm{SE}$ on 7 of 10
...
\item \textbf{The combined pipeline, read out by a fixed linear model, matches or exceeds the two
...
the same fixed linear readout, for the shipped encoder, its dynamics-free twin and an untrained
...
than they add: the engineered basis is already saturated (oscillating giants, $-0.003$ on a baseline
of $0.920$) or the training set is too small to pay for them (RGB versus helium-burning, $-0.051$ on
755 training stars). On both, an untrained-encoder control loses by an amount we cannot resolve from
the trained fusion's, the signature of collinear columns diluting a linear fit, not of $\mu$ carrying
misleading content (Appendix~\ref{app:negatives}). Across all ten tasks the untrained control is
...
\paragraph{The fusion probe matches or exceeds the two supervised networks.} Against C1, which shares
...
seven tasks the fusion probe recovers $0.98$--$1.09$ of its margin above the metric floor.
